...
<style>
    .plot-full {
        background-color: #ffcccc; /* Light red for full plots */
        color: #a00; /* Dark red text */
    }
    .plot-available {
        background-color: #ccffcc; /* Light green for available plots */
        color: #0a0; /* Dark green text */
    }
</style>
@endsection
...
<div class="top-controls">
    <div class="search-wrapper">
        ...
        <div class="button-group">
            ...
            <button id="toggle-availability" class="toggle-button" onclick="window.location.href='{{ url('/availability') }}'">View Availability</button>
        </div>
...
<div class="plot-list">
    @foreach($plots as $plot)
        @php
            $statusClass = $plot->isFull ? 'plot-full' : 'plot-available';
            $statusText = $plot->isFull ? 'Full' : 'Available';
        @endphp
        <div class="plot-item {{ $statusClass }}">
            <div>Plot ID: {{ $plot->Plot_ID }} - {{ $statusText }}</div>
            <small>Capacity: {{ $plot->capacity }}, Current Usage: {{ $plot->current_usage }}</small>
        </div>
    @endforeach
</div>
@endsection
...
<script>
    ...
    // Define maximum capacities for each plot type
    const maxCapacities = {
        '1': 1, // Apartment
    ...
    // Function to check if a plot is full
    function isPlotFull(plotType, currentCount) {
        const maxCapacity = maxCapacities[plotType];
        return currentCount >= maxCapacity;
    }

    // Update the displaySearchResults function to include plot full check
    function displaySearchResults(data) {
        ...
        // Create a count object to track the number of deceased per plot type
        const plotCounts = {};

        // First, count the number of deceased for each plot type
        data.forEach(person => {
            if (!person || !person.plot_type) return;
            const plotType = person.plot_type;
            plotCounts[plotType] = (plotCounts[plotType] || 0) + 1;
        });
        ...
        data.forEach(person => {
            ...
            // Check if the plot is full
            const currentCount = plotCounts[plot_type] || 0;
            const plotStatus = isPlotFull(plot_type, currentCount) ? 'Full' : 'Available';
            const statusClass = plotStatus === 'Full' ? 'plot-full' : 'plot-available'; // Conditional class

            // Display plot status in the results
            resultsContainer.innerHTML += `
                <div class="search-result-item ${statusClass}" 
                    ...
                    <div>${person.FirstName || ''} ${person.MiddleName ? person.MiddleName.charAt(0) + '.' : ''} ${person.LastName || ''} - ${plotStatus}</div>
                    <small>${plot_type ? getPlotType(plot_type) : ''} - Block ${person.block || ''}, Section ${person.section || ''}, Row ${person.row || ''}, Plot ${person.plot || ''}</small>
                </div>
            `;
        });
    ...
    document.getElementById('search-results').addEventListener('click', async function(e) {
        ...
        try {
            ...
            // Initialize carousel and scroll to the correct slide
            initializeCarousel();
            if (targetIndex > 0) {
                // Find the carousel and scroll to the target slide
